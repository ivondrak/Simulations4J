...
public class Graph {
	...
	public void update(float xValue, float yValue, char symbol) {
		...
		if (xValue < minXValue || xValue > maxXValue)
			System.out.println("X = "+Float.valueOf(xValue).toString()+" out of the range for this graph!");
		else
			if (yValue < minYValue || yValue > maxYValue)
				System.out.println("Y = "+Float.valueOf(yValue).toString()+" out of the range for this graph!");
			else {
	...
	public void show() {
		...
		for (int i=0; i < (maxXCoord-Float.valueOf(maxXValue).toString().length()); i++)
			System.out.print(space);
		...
		System.out.println();
		yLabel = minYValue;
		String label = Float.valueOf(yLabel).toString();
